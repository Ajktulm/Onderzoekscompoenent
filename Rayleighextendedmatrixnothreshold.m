...
for i = nRuns/5:nRuns
    ...
    for k = 1:T-1
        ...
        if(Increasing == true)
           if(Estimateddecisionmatrixincreasing(Estimatedstatenumber,k) == 1) 
               Algocosts(i) = 1/Run(k); 
               Sellincreasing = Sellincreasing + 1;
               break
           end
        else
            if(Estimateddecisionmatrixdecreasing(Estimatedstatenumber,k) == 1) %Making threshold much less severe results in significantly lower costs;
               Algocosts(i) = 1/Run(k); 
